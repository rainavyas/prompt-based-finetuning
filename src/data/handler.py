...
from .biasing import create_balanced_data, create_balanced_length_biased_data, \
                     create_balanced_lexical_biased_data, create_balanced_inverse_lexical_bias
...
class DataHandler:
    ...
    def prep_split(self, data_name:str, mode:str, bias:str=None, lim=None):
        split = self.load_split(data_name=data_name, mode=mode, bias=bias, lim=lim)
        if is_classification(data_name):
            data = self._prep_ids(split)
        elif is_nli(data_name):
            data = self._prep_ids_pairs(split)
        else: raise ValueError(f"invalid data set: {data_name}")
        return data
        #split_id = {'train':0, 'dev':1, 'test':2}
        #data = self.prep_data(data_name, bias, lim)[split_id[mode]]
    ...
    def load_split(cls, data_name:str, mode:str, bias:str=None, lim=None):
        ...
        if bias is None: 
            ...
            data = cls.bias_data(data=data, bias=bias, lim=lim)

        return data
        #data = cls.load_data(data_name, bias, lim)[split_index[mode]]
